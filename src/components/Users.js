...
class Users extends Component{
    ...
    editUser = e => {
        ...
        user.user_name = this.state.user_name !== "" ?
            this.state.credentials.user_name : undefined;
        user.password = this.state.password !== "" ?
            this.state.credentials.password : undefined;
        ...
        return ( <div className="user-profile">
            <div className="user-edit">
                <p className="user-name">
                    { this.props.user.user_name }'s Profile
                </p>
                <button className="btn btn-user"
                        onClick={ this.toggleEdit }>
                    Edit Profile
                </button>
                { this.state.open ? ( <form onSubmit={ this.editUser }>
                    <input
            ...
            { this.props.userPost.length === 0 ?
                ( <p className="no-post"> No post yet! Be sure to post your
                    stories!</p> ) : ( <div className="user-profile-posts">
                    <h3 className="user-profile-current">My Current
                        Posts: </h3>
                    { this.props.userPost.map( user => ( <div key={ user.id }>
                        <button
...
const mapStateToProps = state => {
    ...
    return {
        user: state.usersReducer.user, userPost: state.postsReducer.posts
    };
};
